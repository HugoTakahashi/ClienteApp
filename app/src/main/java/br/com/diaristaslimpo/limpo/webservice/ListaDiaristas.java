...
import br.com.diaristaslimpo.limpo.telas.TelaSExibeDiarista;

/**
 * Created by user on 24/04/2016.
 */
public class ListaDiaristas extends AsyncTask<String, Void, String> { // linha 22
    ...
    public ListaDiaristas(Context context) {
    ...
    protected String doInBackground(String... params) {// thread em segundaria
        ...
        try {
            ...
            final JSONArray recebe = requester.conexaoArray("http://limpo-dev.sa-east-1.elasticbeanstalk.com/api/Diarista/ListaDiaristaDisponiveis", json2);
            for(int i=0;i < recebe.length();i++) {
                ...
                requester = new ConectaWS();
                final JSONObject maps = requester.conexao(url, "");
    ...
    protected void onPostExecute(String resposta) { // thread principal
        ...
        if (resposta == null) {
            dialog.dismiss();
            Intent it = new Intent(context, TelaSExibeDiarista.class);
